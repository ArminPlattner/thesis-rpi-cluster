...
parser = argparse.ArgumentParser(description="Experiment 5 (DRAIN + INT8) Inference Engine")
parser.add_argument("expected_anomalies", type=int, help="Expected anomaly count to process")
args = parser.parse_args()
TOTAL_EXPECTED_ANOMALIES = args.expected_anomalies
...
# Drain template miner initialization
...
if os.path.exists(state_bin):
    ...
    print(f"No drain_state.bin found; creating new TemplateMiner.")
...
print("Quantized INT8 ONNX + Drain engine initialized successfully.")
...
def print_thesis_report():
    ...
    success_rate_iac = (metrics["correct_iac_classifications"] / metrics["processed_count"]) * 100 if metrics["processed_count"] > 0 else 0.0
    success_rate_fault = (metrics["correct_fault_classifications"] / metrics["processed_count"]) * 100 if metrics["processed_count"] > 0 else 0.0
    ...
    print("\n\n" + "=" * 60)
    print(f"GENERATED THESIS METRICS REPORT (EXP 5 DRAIN+INT8 STRESS RUN: {metrics['processed_count']})")
    print("=" * 60)
    print(f"| Metric                          | Value                         | Statistical Scale / Context |")
    print(f"| :---                            | :---                          | :---                        |")
    print(f"| Total Anomalies Evaluated       | {TOTAL_EXPECTED_ANOMALIES:<29} | Gatekeeper-forwarded only   |")
    ...
    print(f"| Classification IaC Accuracy     | {success_rate_iac:<29.1f} % | Quantized IaC accuracy      |")
    print(f"| Classification Fault Accuracy   | {success_rate_fault:<29.1f} % | Quantized fault accuracy    |")
...
consumer = Consumer({
    ...
    "group.id": f"brain-exp5-drain-int8-{int(time.time())}",
    "auto.offset.reset": "earliest",
})
producer = Producer({"bootstrap.servers": KAFKA_BOOTSTRAP})
consumer.subscribe([IN_TOPIC])

print(f"DRAIN+INT8 engine active. Listening for exactly {TOTAL_EXPECTED_ANOMALIES} forwarded anomalies.")
...
try:
    while metrics["processed_count"] < TOTAL_EXPECTED_ANOMALIES and not _graceful_stop:
        ...
        if msg is None:
            if metrics["processed_count"] > 0:
                ...
                if empty_polls_count >= MAX_EMPTY_POLLS:
                    print(f"\nStream idle timeout hit. Shutting down.")
                    break
            continue
        ...
        true_iac = payload.get("true_iac", payload.get("true_iac_category", "Unknown"))
        true_fault = payload.get("true_fault", payload.get("true_fault_category", "Unknown"))
        ...
        print(f"\n--- {'HEALABLE' if analysis['is_healable'] else 'MANUAL'} (DRAIN + INT8 ONNX) ---")
