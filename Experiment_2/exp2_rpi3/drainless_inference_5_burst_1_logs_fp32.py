...
class MultiHeadTinyBERT(nn.Module):
    def __init__(self, num_iac, num_fault):
        ...
        # Initialize from an empty configuration object to completely bypass file-searching OS errors
...
if os.path.exists(weights_file):
    # Load the unified dictionary layer cleanly with strict verification tracking
...
try:
    while True:
        ...
        # DRAINless architecture passes the cleaned raw log directly into model inference
        cleaned_input = clean_log(raw_log)
        analysis = predict_with_safety_gate(raw_log, cleaned_input)
        ...
        print(f"\n--- {status} (DRAINLESS NATIVE PYTORCH FP32) ---")
        print(f"Msg ID:                    {msg_id}")
        print(f"Queue + transport time:    {queue_to_arrival_ms:.2f} ms")
        print(f"Inference processing time: {analysis['inference_ms']:.2f} ms")
