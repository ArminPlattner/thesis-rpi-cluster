...
config.masking_instructions = [
    ...
    # We keep TASK_NAME but removed PLAY_NAME as we filter those lines out for preprocessing
...
def get_template(log_line):
    ...
    # Filter out PLAY headers and empty lines, then strip whitespace
    filtered_lines = [
        line.strip() for line in lines
        if not line.strip().startswith("PLAY") and line.strip()
    ]

    # Join into a single-line string (re-flattening)
    flattened_log = " ".join(filtered_lines)

    # Extract template via Drain3
...
# FULL 32-BIT FLOATING POINT VERSION 
...
"""
TESTING THE FLOATING-POINT 32-BIT VERSIONS
"""
import os
import io
import json
...
# UPLOAD TEST DATASET (5,000 rows)
uploaded = files.upload()
...
# SETUP - Install missing dependency
...
# STEP 1: EXPORT (WITH DYNAMIC BATCH AXIS)
...
# DYNAMIC UPLOAD TARGET EVALUATION DATASET
print("Please select your test dataset file")
uploaded = files.upload()
test_filename = list(uploaded.keys())[0]
test_df = pd.read_csv(io.BytesIO(uploaded[test_filename]))
...
# HIGH-VELOCITY MINIBATCH EVALUATION
all_pred_iac = []
all_pred_fault = []
batch_size = 32
...
# Filter out rows where the joint prediction failed
...
# Show where the errors are clustered
...
# Export to Google Drive for detailed reading/LibreOffice mapping
...
# CALCULATE & PRINT FORMATTED RESULTS
test_df["pred_iac"] = all_pred_iac
test_df["pred_fault"] = all_pred_fault
